...
Проект ориентирован на сценарий параметрического эскизника:

- пользователь создаёт точки, линии, окружности и дуги;
- CAD добавляет геометрические ограничения;
- решатель пересчитывает положение зависимых объектов;
- приложение забирает обновлённые координаты и отображает новый эскиз.
...
Все примитивы можно создавать:

- либо напрямую по координатам;
- либо через уже существующие опорные точки по их `ID`.
...
- ограничение для дуги `arc center on perpendicular`;
- фиксация точки;
- фиксация линии;
- фиксация окружности.

`LineInCircle` уже присутствует в перечислениях и дескрипторах, но через единый `addRequirement()` в текущей версии ещё не поддержан.
...
То же касается окружностей и дуг: после решения можно прочитать новые координаты опорных точек и радиус.
...
Почему так: библиотека уже собирается как `OurPaintDCM`, но для внешнего CAD сейчас проще и надёжнее явно передать include-пути, которые используются внутри проекта.
...
`OurPaintDCM` хорошо подходит, если вам нужен:

- компактный 2D DCM для собственного CAD/эскизника;
- контролируемый C++ API без тяжёлого внешнего рантайма;
- доступ не только к `solve()`, но и к внутренним ограничениям, невязкам и диагностике;
- основа для дальнейшего развития параметрического редактора.

## Ограничения текущей версии

- только 2D;
- нет готового install/export-пакета для системной установки;
- `LineInCircle` пока не доведён до рабочего состояния в unified API;
